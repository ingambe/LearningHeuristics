...
from instance_reader import Machine, Task, CapacityManager2, Job, PenaltyManager, perf_measures


def beam_search(task_list: List[Task], machine_list: List[Machine], job_list: List[Job], setup, cm: CapacityManager2, obj_dv=None):
    ...
    with torch.no_grad():
        ...
        master_net.eval()
        output_iter = ray.get([master_iter.remote(pm_ray, deadlines_ray, cm_ray, bijection_job_id_ray,
                                                   bijection_task_id_ray, number_jobs, number_tasks, all_jobs_ray,
                                                   job_weights_ray, job_list_ray, master_net)])
        ...
        output_iter = ray.get([one_pop_iter.remote(pm_ray, deadlines_ray, cm_ray, bijection_job_id_ray,
                                                   bijection_task_id_ray, number_jobs, number_tasks, all_jobs_ray,
                                                   job_weights_ray, job_list_ray, master_net, k[0], k[2]) for k in list_choice_made])
    ...
    p['time'] = [t.to_json() for t in task_list]
    return dict(machine_days=machine_days, **p)

@ray.remote
def master_iter(pm, deadlines, cm, bijection_job_id, bijection_task_id, number_jobs, number_tasks, all_jobs, job_weights, job_list, network):
    with torch.no_grad():
        machine_days = defaultdict(lambda: defaultdict(lambda: 0))
        ...
        while number_finished_job < number_jobs:
            ...
            if job_to_allocate_this_step.shape[0] > 0:
                if len(job_to_allocate_this_step) == 1:
                    ...
                    job_prob_value, order_jobs = torch.sort(probabilities, descending=True, stable=True)
                    ...
                    idx_prob = 0
                    while torch.isclose(prob1, prob2).item() and idx_prob + 1 < job_prob_value.shape[0]:
                        idx_prob += 1
...
@ray.remote
def one_pop_iter(pm, deadlines, cm, bijection_job_id, bijection_task_id, number_jobs, number_tasks, all_jobs, job_weights, job_list, network, greedy_until=-1, swap_with=1):
    with torch.no_grad():
        machine_days = defaultdict(lambda: defaultdict(lambda: 0))
            ...
            if job_to_allocate_this_step.shape[0] > 0:
                if len(job_to_allocate_this_step) == 1:
                    ...
                    if greedy_until == step_nb:
                        ...
                        order_jobs[0], order_jobs[swap_with] = order_jobs[swap_with], order_jobs[0]
                step_nb += 1
                for i in order_jobs:
                    job_object = all_jobs[i]
                    # check if compatible
