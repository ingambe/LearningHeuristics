...
def solve_with_es(
    ...
    with torch.no_grad():
        ...
        adam = Storage.adam
        while ("timeLimit" in setup and start + setup["timeLimit"] > time.time()) or (
            "timeLimit" not in setup and i < iter_nb
        ):
            alpha = Storage.alpha
            sigma = Storage.sigma
            ...
            print(
                f"iteration {i} best solution found this iter {results[best_pop] + obj_dv}, with a mean of {mean_result + obj_dv}, best solution so far {best_solution[0] + obj_dv}"
            )
            ...
            wandb.log(
                {
                    f"{instance_name}/best_iter_solution": results[best_pop] + obj_dv,
        ...
        while number_finished_job < number_jobs:
            ...
            if job_to_allocate_this_step.shape[0] > 0:
                if len(job_to_allocate_this_step) == 1:
                    ...
                    # print(probabilities)
                    order_jobs = np.random.choice(
                        job_to_allocate_this_step,
                        size=len(job_to_allocate_this_step),
                        replace=False,
