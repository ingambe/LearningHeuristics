...
class TrainNetwork(CpSolverSolutionCallback):
    ...
    def __init__(self, all_tasks, job_list, pm,
                 deadlines,
                 cm,
                 bijection_job_id,
                 bijection_task_id,
        ...
        self.network = Network(6, 1)
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=0.0003, betas=(0.9, 0.999))
    ...
    def on_solution_callback(self):
        ...
        for job in self.job_list:
            for task in job.tasks:
                ...
                task.solver_result_processing_day = self.Value(cp_task.start)
        ...
        all_jobs_copy = deepcopy(self.all_jobs)
        job_list_copy = deepcopy(self.job_list)
        one_pop_iter(
            self.network,
            self.optimizer,
            self.pm,
        ...
        print(f'Current solution: {self.__solution_count}')
...
def solve_with_cp_get_result(
    ...
    print("Start solving")
    special_callback = TrainNetwork(all_tasks, job_list,
                                    pm,
                                    deadlines,
                                    cm,
                                    bijection_job_id,
...
def generate_data(
    ...
    days_allocation, task_day = solve_with_cp_get_result(job_list,
                                                         setup,
                                                         cm,
                                                         pm,
                                                         deadlines,
...
def one_pop_iter(
    ...
    with torch.no_grad():
        ...
        while day <= max(days_allocation.keys()):
            ...
            for job_nb in job_to_allocate_this_step:
                ...
                if task_day[task.id] >= max(day - 1, 0):
            ...
            for machine_id, machine_job_list in machine_to_allocate.items():
                ...
                if job_to_allocate_this_step_machine.shape[0] > 0:
                    ...
                    #indexes_sort_deadline = torch.argwhere(delay_deadlines <= max(delay_deadlines.min(), 0) + 7).view(
                    #    -1)
    ...
    # save the model
    torch.save(network.state_dict(), 'pretrained.pt')
