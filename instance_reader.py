...
class Machine:
    ...
    def next_timestep(self, t, cm):
        ...
        while self.capacity(t + i, cm) == 0:
            ...
            if i > 1000:
                raise Exception(
                    f"No capacity in next 1000 day for machine {self.id} and day {t + self.day_zero}"
                )
        return t + i
...
class Job:
    ...
    def result_delay(self):
        return (
            max(0, self.tasks[-1].result_processing_day - self.deadline)
            if len(self.tasks) > 0
            else 0
        )
...
class PenaltyManager:
    def __init__(self, objective):
        self.project_weights = {
            a["project"]: a["jobWeight"] for a in objective["projects"]
        }
    ...
    def project(self, project):
        return (
            self.project_weights[project]
            if project in self.project_weights
